...
SELECT category, SUM(rating)/COUNT(rating) FROM analytics GROUP BY category ORDER BY SUM(rating)/COUNT(rating) DESC;
...
SELECT app_name, reviews, min_installs, min_installs * 1.0 / reviews FROM analytics WHERE min_installs >= 100000 ORDER BY min_installs / reviews * 1.0 DESC;
...
SELECT app_name, rating FROM analytics x WHERE min_installs > 50000 AND rating = (SELECT MAX(rating) FROM analytics y WHERE min_installs > 50000 AND x.category = y.category);
...
SELECT app_name FROM analytics ORDER BY array_position(genres, 'Education') >= 0;
